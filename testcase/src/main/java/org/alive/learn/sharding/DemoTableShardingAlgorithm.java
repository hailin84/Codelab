...
public class DemoTableShardingAlgorithm implements SingleKeyTableShardingAlgorithm<Integer> {
	...
	/**
	 * select * from t_order from t_order where order_id = 11 й╕йдйд SELECT * FROM
	 * t_order_1 WHERE order_id = 11 select * from t_order from t_order where
	 * order_id = 44 й╕йдйд SELECT * FROM t_order_0 WHERE order_id = 44
	 */
	...
	/**
	 * select * from t_order from t_order where order_id in (11,44) й└йдйд SELECT *
	 * FROM t_order_0 WHERE order_id IN (11,44) й╕йдйд SELECT * FROM t_order_1
	 * WHERE order_id IN (11,44) select * from t_order from t_order where
	 * order_id in (11,13,15) й╕йдйд SELECT * FROM t_order_1 WHERE order_id IN
	 * (11,13,15) select * from t_order from t_order where order_id in
	 * (22,24,26) й╕йдйдSELECT * FROM t_order_0 WHERE order_id IN (22,24,26)
	 */
	...
	/**
	 * select * from t_order from t_order where order_id between 10 and 20 й└йдйд
	 * SELECT * FROM t_order_0 WHERE order_id BETWEEN 10 AND 20 й╕йдйд SELECT *
